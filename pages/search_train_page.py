...
class SearchTrainsPage:
    ...
    def search_trains(
        ...
        self.page.locator(
            f"//li[@id='p-highlighted-option']//span[contains(., '{source.upper()}')]"
        ...
        self.page.locator(
            f"//li[@id='p-highlighted-option']//span[contains(., '{destination.upper()}')]"
        ).wait_for(state="visible")
        ...
        if classes:
            ...
            self.page.locator(
                f"(//li[@role='option' and contains(@aria-label,'{classes}')])[1]"
            ).wait_for(state="visible")
        ...
        if general:
            ...
            self.page.locator(
                f"(//li[@role='option' and contains(@aria-label,'{general}')])[1]"
            ).wait_for(state="visible")
        ...
        try:
            self.page.locator(self.NO_DIRECT_TRAIN_POP_UP).wait_for(
                state="visible",
                timeout=2000
            )
            ...
            logger.info("Not an indirect train or pop-up is not visible")
